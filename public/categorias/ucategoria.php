...
$id=$_GET['id'];
$datosCategorias=(new Categoria)->devolverCategoria($id);

if(isset($_POST['btnUpdate'])){
    $nomb=trim(ucfirst($_POST['nombre']));
    $desc=trim($_POST['descripcion']);
    (new Categoria)->setId($id)
    ->setNombre($nomb)
    ->setDescripcion($desc)
    ->update();
...
<html lang="es">
    ...
    <body style="background-color:#7CFF7E">
        <h3 class="text-center">Actualizar categoría (<?php echo $datosCategorias->id ?>)</h3>
        <div class="container mt-2">
            <div class="bg-primary p-4 text-white rounded shadow-lg m-auto" style="width:35rem">
                <form name="cpost" action="<?php echo $_SERVER['PHP_SELF']."?id=$id"; ?>" method='POST'>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="n" name="nombre" value="<?php echo $datosCategorias->nombre ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="d" class="form-label">Descripción:</label>
                        <textarea class="form-control" id="d" rows="3" required name="descripcion"><?php echo $datosCategorias->descripcion ?></textarea>
                    </div>
                    <div>
